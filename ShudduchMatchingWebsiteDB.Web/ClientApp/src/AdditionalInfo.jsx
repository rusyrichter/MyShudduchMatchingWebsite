...
const AdditionalInfo= () => {
    ...
    const loadConfirmed2 = async () => {
        const { data } = await axios.get(`/api/shudduchMatching/getFileDownloaded?boyid=${id}`);
        if (data) {
    ...
    const onUpdateClick = async (string) => {
        ...
        await axios.post('/api/shudduchMatching/updateAdditionalInfo', {
            lookingFor,
            personality,
            contactInfo,
            id
    ...
    const onUploadClick = async () => {
        ...
        await axios.post('/api/shudduchMatching/upload', { base64, name: file.name, boyid: id });
        setNothingUploaded(false);
    }
    ...
    const onUploadAnotherResume = async () => {
        const { data } = await axios.get(`/api/shudduchMatching/getFileDownloaded?boyid=${id}`);
        const name = data;
        await axios.post('/api/shudduchMatching/deleteupload', { boyid : id, name});
        setNothingUploaded(true);
    }
    ...
    const onDownloadClick = async () => {
        ...
        if (!nothingUploaded) {
            const { data } = await axios.get(`/api/shudduchMatching/getFileDownloaded?boyid=${id}`);
            window.location.href = `/api/shudduchMatching/download?fileName=${data}`;
        } else {
            const file = fileRef.current.files[0];
            window.location.href = `/api/shudduchMatching/download?fileName=${file.name}`;
        }
    }
    ...
    return (
        <>
            ...
            <div style={cardStyle}>             
                <Link to={`/addViewReferences/${id}`} type="button" className="btn btn-light mt-4"> Add/View References </Link>
            </div>
            ...
            <div style={{ ...cardStyle, padding: '20px', marginTop: '20px', marginBottom: '20px' }} className='row'>
                <Link to={`/uploadDownloadPicture/${id}`} type="button" className="btn btn-light mt-4">Download or Upload {firstName}'s Picture</Link>
            </div>
